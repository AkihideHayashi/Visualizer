...
/**TODO
 * meshesはいまいちなので、rendered_atoms: Array<THREE.Mesh>にする。
 * draged_atomはいまいちなので、selected_atoms: Array<number>にする。
 * そうするとdragged_indexは不要なのでなくす。
 * indexという名前はいまいちなのでframeにする。
 * modeを追加した影響でmouse_is_downは不要になったので消す。
 *  */ 

class ThreeControler{
  ...
  atoms: Array<THREE.Mesh>;
  bonds: Array<THREE.Mesh>;
  plane: THREE.Plane;
...
class Intersection {
  ...
  intersect_objects(mouse: THREE.Vector2, camera: Camera, meshes: THREE.Mesh[]){
    this.raycaster.setFromCamera(mouse, camera);
    const intersects = this.raycaster.intersectObjects(meshes);
    return intersects;
  }

  grab_nearest(mouse: THREE.Vector2, camera: Camera, meshes: Array<THREE.Mesh>){
    const intersects = this.intersect_objects(mouse, camera, meshes);
    if (intersects.length > 0){
      const draged = intersects[0].object;
      this.normal.set(0.0, 0.0, -1.0);
      this.normal.applyQuaternion(camera.quaternion);
      this.plane.setFromNormalAndCoplanarPoint(this.normal, draged.position);
      if (this.raycaster.ray.intersectPlane(this.plane, this.intersection)){
        this.offset.copy(this.intersection).sub(draged.position);
      }
    }
